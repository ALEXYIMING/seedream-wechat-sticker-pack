...
1. 读取参考图（通常是旧项目的 `banner.png`、`赞赏引导图.png`、`赞赏致谢图.png`），理解它的色彩、构图、完成度和“专辑物料”属性。
2. 用 `byted-ark-seedream-skill/scripts/generate.js` 的 `image-to-image` 重新生成三张独立设计稿，`--watermark false`，参考图只用于风格和版式，不复制内容。
3. Prompt 必须明确：不要拼贴已有主表情、不要堆缩略图、不要真实人物、不要文字；要全新视觉主体，例如 8Bit 街机招牌、投币恋爱机、糖果售卖机、爱心宝箱、像素奖杯等。
4. 让 Seedream 预留标题牌/文案牌区域，生成后用本地 PIL 叠加中文，确保文字准确。
5. 因 Seedream 尺寸下限，先高分辨率生成，再本地裁切/缩放成微信规格：`banner 750×400`、`赞赏引导 750×560`、`赞赏致谢 750×750`。
6. 更新 `配套图/` 和 `打包/<album-name>_微信表情物料.zip`。
...
- 精品档/用户给参考时：`banner`、`赞赏引导图`、`赞赏致谢图` 不能只是把主表情堆砌在一起；应参考用户给的旧专辑物料，使用 Seedream 重新生成独立设计稿（例如复古街机招牌、投币恋爱机、爱心宝箱/奖杯等全新视觉主体），再本地叠加必要中文，避免 AI 乱码。`cover` 和 `icon` 可继续从主表情或封面视觉裁切。

精品档注意：Seedream 直接生成微信规格低像素图可能报错（像素总数不足），应先用高分辨率同构图生成，再本地裁切/缩放到微信规格，例如 `banner` 先用 `2560x1440` 后裁到 `750×400`，`赞赏引导图` 先用约 `2304x1720` 后裁到 `750×560`，`赞赏致谢图` 用 `2048x2048` 后缩到 `750×750`。
